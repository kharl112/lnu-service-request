...
export default {
  ...
  computed: {
    loading() {
      return this.$store.getters["notification/getLoading"].admin;
    },
    notifications() {
  ...
  methods: {
    ...
    markAllAsRead() {
      this.$store.dispatch("notification/markAllAsRead", "admin");
    },
    getFullname(name) {
                ...
                <v-list-item-avatar>
                  <v-avatar :color="getAvatarColor(item.user.user_type)">
                    <span class="white--text headline">
                      {{ getAdminInitials(item.initiator) }}
                    </span>
                  </v-avatar>
                </v-list-item-avatar>
                <v-list-item-content>
